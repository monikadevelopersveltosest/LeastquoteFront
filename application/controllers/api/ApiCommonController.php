...
public function user_registration_post()
    {
       ...
       if ($this->form_validation->run() == FALSE) {
           ...
           $response = array('status' => false, 'message' =>  validation_errors());
            ...
            $check_user = $this->Common_model->getSingleRecordById('users', array('username	' => $_POST['username']));
             if(empty($check_user)){
         ...
         if(!empty($add_otp)){
              $check_user = $this->Common_model->getSingleRecordById('users', array('subscriber_id	' => $add_otp));
            $this->response(['status' => true, 'message' => "User successfully registered.",'data'=>$check_user], REST_Controller::HTTP_OK);   
         }else{
             $responseArray = array('status' => FALSE, 'message' => 'Opps somthing went wrong.');
             $this->response($responseArray,REST_Controller::HTTP_OK);
            }
        }
    }
...
/* seller login */
        ...
        if($this->form_validation->run() == FALSE){
           ...
           $data=array();
            ...
            if($res['status']!=0){
                ...
                $this->response(['status' => false, 'message' => "Please verify to the admin after you able to login.",'data'=>''], REST_Controller::HTTP_OK);
            }else{
    ...
    public function virtualCash_post(){
        ...
        $this->form_validation->set_rules('upi-id', 'Up id', 'required');
        $this->form_validation->set_rules('saller_id', 'saller id', 'required');
         if ($this->form_validation->run() == FALSE) {
            $errors = strip_tags(validation_errors());
            $response = array('status' => false, 'message' =>  validation_errors());
            $this->response($response);
            ...
            $insert_data['amount']=$_POST['upi-id'];
            $insert_data['saller_id']=$_POST['saller_id'];
            $insert_data['created_date']=date('Y-m-d');
            
             $check_saller = $this->Common_model->GetWhere('shops', array('shop_id' => $_POST['saller_id'],'status'=>1));
    ...
    public function virtualcash_get()
    {
        $allvirtuallist = $this->db->get_where("vcashSaller")->result();
        if ($allvirtuallist) {
            $this->response(['success' => true, 'message' => "virtual all list successfully.", 'data' => $allvirtuallist], REST_Controller::HTTP_OK);
        } else {

            $this->response(['success' => false, 'message' => "Record not found.", 'data' => ''], REST_Controller::HTTP_NOT_FOUND);
        }
    }
   public function citiylist_get()
    {
        $allcitylist = $this->db->get_where("city_list")->result();
        if ($allcitylist) {
            $this->response(['success' => true, 'message' => "City list successfully.", 'data' => $allcitylist], REST_Controller::HTTP_OK);
        } else {
    ...
    public function userEdit_post()
    {
        ...
        if($this->form_validation->run() == FALSE){
            ...
            $this->response($response);
             ...
             $userData = $this->Common_model->getSingleData('users', array('subscriber_id' => $_POST['user_id']));
             if(!empty($userData)){
                 $post_data=array();
                 $post_data['username']=!empty($_POST['username']) ? $_POST['username'] : $userData['username'];
                 $post_data['user_contact_number']=!empty($_POST['user_contact_number']) ? $_POST['user_contact_number']:$userData['user_contact_number'];
                 $update = $this->Common_model->updateRecords('users', $post_data, array('subscriber_id' => $_POST['user_id']));
                  $userData = $this->Common_model->getSingleData('users', array('subscriber_id' => $_POST['user_id']));
