                ...
                <div class="col-sm-7" >
                    ...
                    <div class="w-100 bg-light border" style="border:1px #999 solid; padding:30px; width:100%; box-shadow: 10px 10px 10px rgba(0, 0, 0, 0.10); float:left;">
                        <div class="w-100">
                            ...
                            <!--<select class='form-control mb-3' id='testSelect1' multiple >-->
...
<aside class="main-sidebar">
  ...
  <section class="sidebar">
    ...
    <ul class="sidebar-menu" data-widget="tree">
      ...
      <li class="<?php echo (checkTabActive(array("orderhistory"))) ? "active" : ""; ?>"> <a href="<?php echo base_url();?>shop/Order history"><i class="fa fa-cart-arrow-down"></i> <span>Order histroy</span></a> </li>
...
<script>
     ...
     function myfunction2(){ 
        ...
        if(public_id!=""){
            ...
            console.log(res);
                  if(res.seller_data!="null"){
                     ...
                     <?php 
                            $shop = $this->session->userdata(SHOP_SESSION);
                            $productlistdat=$this->Common_model->GetWhere('product', array("shop_id"=>$shop['shop_id']));
                            //echo "<pre>";print_r($productlistdat);die;
                            if(!empty($productlistdat)){
                         ?>
                        html+="<div class=''>";
                        html+="<select class='form-control color-var-select js-example-basic-multiple' id='testSelect1' onclick='changeproduclist("+res.seller_data.pablic_id+")' style='border:1px #F00 solid;' aria-describedby='basic-addon1' multiple>";
	    				<?php if(isset($productlistdat) && !empty($productlistdat)){ 
	    					foreach($productlistdat as $val){
	    				?>
	    				html+="<form id='form_calc'>";
	    				html+="<option  value='<?php echo $val['product_id'];?>' data-id='<?php echo $val['pream_points']; ?>' data-val='<?php echo $val['unit_price']; ?>'><?php echo $val['name']; ?></option>";
	    			<?php } } ?>
	    			html+="</form>";
	    			html+="</select>";
	    			//html+="<div id='price' class='alert alert-info mt-3'>Total Price: $ <span >0</span></div>";
	    			<?php 
	    			$total=0;
	    			$total1=0;
	    			if(isset($productlistdat) && !empty($productlistdat)){ 
	    					foreach($productlistdat as $val){
	    					    $total =$val['unit_price'];
	    					    $total1 += $val['unit_price'];
	    				?>
	    			 $(document).ready(function() {
                            document.multiselect('#testSelect1').setCheckBoxClick("checkboxAll", function(target, args) {
								var total=0;
		                        $("#testSelect1 option").each(function(){
		                              var pid=$(this).val();
		                              var pr=$(this).text();
		                              var price=$(this).attr("data-val");
		                               total = total+parseFloat(price);
		                              var preamspoint=$(this).attr("data-id");
		                          if(args.checked==true){
		                      			var html1="<table width='100%' class='datp' border='0' style='font-size:16px;' id='' data-id="+pid+"><tbody><tr style='padding-bottom:120px;'><td id='publicid1' style='font-weight:bold;'>Product Name</td><td id='publicid1' style='font-weight:bold;'>Price</td><td  style='font-weight:bold;'>Preams Points</td></tr><tr id='details"+pid+"'><td id='product"+pid+"'> "+pr+"</td><td id='price"+pid+"'> "+price+"</td><td><div class='sp-quantity"+pid+"'><div class='sp-minus fff cart-qty-minus"+pid+"'><a class='ddd' href='#' data-multi='-1'>-</a></div><div class='sp-input'><input type='text' class='quntity-input"+pid+"' value='"+preamspoint+"' /></div><div class='sp-plus fff cart-qty-plus"+pid+"'><a class='ddd' href='#' data-multi='1'>+</a></div></div></td></tr></tbody></table>";
		                                //$(".addselect").append("<span id='poduct' class='produ"+pid+"'><span id='"+pid+"'>"+pr+"</span>&nbsp;<div class='w-100' style='margin-top:0px; padding-bottom:20px;margin-left: 66px;'><table width='100%' border='0' style='font-size:16px;' id='preamspoints"+pid+"'><tbody><tr style='padding-bottom:120px;'><td id='publicid1' style='font-weight:bold;'>Price</td><td  style='font-weight:bold;'>Preams Points</td></tr><tr id='details"+pid+"'><td id='price"+pid+"'> "+price+"</td><td>"+preamspoint+"</td></tr></tbody></table></div></span>");
		                         		$(".addselect").append(html1);
		                         		var incrementPlus;
										var incrementMinus;

										var buttonPlus  = $(".cart-qty-plus"+pid);
										var buttonMinus = $(".cart-qty-minus"+pid);

										var incrementPlus = buttonPlus.click(function() {
											//alert('ok1');
											 var $n = $(this)
											 	.closest('.sp-quantity'+pid)
											// 	.parent(".container")
											 	.find("input.quntity-input"+pid);
											 $n.val(Number($n.val())+1 );
											$.ajax({
										        type:"POST",
										        url:"<?php echo base_url()?>shop/updatepoint",
										        data:{id:pid,pnt:$n.val()},
										        dataType:"json",
										        success:function(res){
										         console.log(res);
										         if(res.status==1){
		                       ...
		                       if(total>0 && args.checked==true){
		                           var html="<div class='w-100' style='margin-top:10px; text-align:center;  color:#F00; font-weight:bold ;font-size:16px;'>Amount Spent on this visit - $"+total+"</div>";
                                  ...
                                  html+="<button type='button' class='btn btn-danger' style='padding-left:20px; padding-right:20px;' onclick='myfunction("+res.seller_data.pablic_id+")'>Submit</button>";
		                           ...
		                           var total=0;
		                      var html="<div class='w-100 amount<?php echo $val['product_id'];?>' style='margin-top:10px; text-align:center;  color:#F00; font-weight:bold ;font-size:16px;'>Amount Spent on this visit - $"+total+"</div>";
                                html+="<div class='w-100 amountlist<?php echo $val['product_id'];?>' style='margin-top:20px; text-align:center;  color:#F00; font-weight:bold font-size:16px;'>";
                                html+="<input type='text' class='form-control amount' id='inputEmail3' placeholder='4000' style='border:1px #F00 solid;'></div>";
                                html+="<div class='w-100 amountadd<?php echo $val['product_id'];?>' style='margin-top:20px; text-align:center;  color:#F00; font-weight:bold font-size:16px;'>";
                                html+="<button type='button' class='btn btn-danger' style='padding-left:20px; padding-right:20px;' onclick='myfunction("+res.seller_data.pablic_id+")'>Submit</button>";
		                       $(".addselect").html("");
		                        }
		                      $("#amountadd").html(html); 
                    		console.log("Checkbox 'Select All' was clicked and got value ", args.checked);
                    		}).setCheckBoxClick("<?php echo $val['product_id'];?>", function(target, args) {
		                        var category1Count = $(".multiselect-checkbox");
                				var i;
                				var text=[];
										var price =$("#testSelect1 option:selected").attr("data-val");
										$("#amountadd").html("");
                    		    if(args.checked==true){
		                          var total=0;
		                          $("#testSelect1 option:selected").each(function(){
		                              var pid=$(this).val();
		                              var pr=$(this).text();
		                              var price=$(this).attr("data-val"); 
		                               total = total+parseFloat(price);
                    ...
                    <?php } }?>
                        html+="<span class='addselect'></span>";
                        html+="</div>";
                    <?php } ?>
                    html+="<div id='amountadd'></div>";
            ...
            var html="<div class='row'>";
                     ...
                     html+="</div><p style='color:red;'> Please Fill Customer Pabulic Id</p>";
                     html+="</div></div></div></div>";
                     $("#userdetail").html(html);
                     setTimeout(function(){location.reload()},1000);
        }
    ...
    // function addpoin(point,id){
    // 	var pnt=$(".quntity-input").val();
    // 	 var $button = $(".ddd");
    // 	var $input = $button.closest('.sp-quantity').find("input.quntity-input");
    // 	console.log($input.val());
    	 
    // 	//console.log('point='+pnt+"id="+id);
...
<script>
    ...
    //     alert('ok');
    //     count++;
    //   var pr= $("#testSelect1 option:selected").text();
    //   var price=$("#testSelect1 option:selected").attr("data-val");
    //   var preamspoint=$("#testSelect1 option:selected").attr("data-id");
    //   var pid=$("#testSelect1 option:selected").val();
    //   if(id!="" && pr!="Select Product"){
    //         $(".addselect").append("<span id='poduct'><span id='"+pid+"'>"+pr+"</span>&nbsp;<div class='w-100' style='margin-top:-25px; padding-bottom:20px;margin-left: 66px;'><table width='100%' border='0' style='font-size:16px;' id='preamspoints"+pid+"'><tbody><tr style='padding-bottom:120px;'><td id='publicid1' style='font-weight:bold;'>Price</td><td  style='font-weight:bold;'>Preams Points</td></tr><tr id='details"+pid+"'><td id='price"+pid+"'> "+price+"</td><td>"+preamspoint+"</td></tr></tbody></table></div></span>");
    //       var totalPrice = 0;  values       = [];
    //         $('select option:selected').each(function(){
    //             values.push($(this).attr('data-val'));
    //             totalPrice += parseInt($(this).attr('data-val'));
    //           //totalMoney += parseInt($(this).attr('data-val'));

    //         });
    //         console.log(totalPrice);
    //       var html="<div class='w-100' style='margin-top:10px; text-align:center;  color:#F00; font-weight:bold ;font-size:16px;'>Amount Spent on this visit - $"+price+"</div>";
    //       html+="<div class='w-100' style='margin-top:20px; text-align:center;  color:#F00; font-weight:bold font-size:16px;'>";
    //       html+="<input type='text' class='form-control amount' id='inputEmail3' placeholder='4000' style='border:1px #F00 solid;'></div>";
    //       html+="<div class='w-100' style='margin-top:20px; text-align:center;  color:#F00; font-weight:bold font-size:16px;'>";
    //       html+="<button type='button' class='btn btn-danger' style='padding-left:20px; padding-right:20px;' onclick='myfunction("+id+")'>Submit</button>";
    //   }else{
    //         var html="<div class='w-100' style='margin-top:10px; text-align:center;  color:#F00; font-weight:bold ;font-size:16px;'>Please Select Product</div>";
    //   }
    //     $("#amountadd").html(html);
    // return false;
    // }
