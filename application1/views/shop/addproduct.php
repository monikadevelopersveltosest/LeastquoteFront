	    ...
	    <div class="row">
	    	<div class="col-xs-12" style="background-color: #fff">
                ...
                //         			if(isset($shop_cat_data) && !empty($shop_cat_data)){
        				    ...
        				    // 		$allcategoriesc = $this->Common_model->getwhrcategoiesbycatid($whrcc);
			            		...
			            		?>
    			                  ...
    			                  <div class="form-group">
    			                    ...
    			                    <select class="form-control" name="categories_id" id="category" onchange="categorychange()" required>
    			                      <option value="">Select Any Category</option>
    			                      <?php 
    			                      foreach ($categorylist as $allcategoriesc) {
    			                      ?>
    			                  ...
    			                  <div class="form-group" id="categoryimage">
    			                      <?php
    			                      if(!empty($product_data['categoryimg'])){ 
    			                      ?>
    			                      <label>Category Image</label>
    			                      <br>
    			                      <img src="<?php echo base_url();?>uploads/category/<?php echo $product_data['categoryimg']; ?>" style='width:491px;height:169px'><input type="hidden" name="categoryimg" value="<?php echo $product_data['categoryimg']; ?>">
    			                      <?php 
    			                      }
    			                      ?>
    			                  </div>
			             ...
			             <!-- <div class="form-group">
			                <label>Short Description &nbsp;<b style="color:#ff0000;">*</b></label>
			                            
                            <textarea class="form-control editor" rows="5" name="short_description" id="short_description" value="<?php echo (!empty($product_data['short_description']))? $product_data['short_description']:''; ?>" > <?php echo (!empty($product_data['short_description']))? $product_data['short_description']:''; ?> </textarea>
			            ...
			            <div class="col-md-6">
			                  ...
			                  <input type="text" class="form-control" step="0.01" id="unit_price" name="unit_price" value="<?php echo (!empty($product_data) && !empty($product_data['unit_price']) ? $product_data['unit_price'] : "" )?>" required>
			                </div>
			                 <div class="form-group">
                                <label>Discounted Price</label>
                                <input type="number" class="form-control" id="product_gst" name="product_gst_discount" value="<?php echo (!empty($product_data) && !empty($product_data['product_gst']) ? $product_data['product_gst'] : "" )?>" required>
                            </div>
                            <div class="form-group">
                                <label>Pream points</label>
                                <input type="number" class="form-control" name="pream_points" value="<?php echo (!empty($product_data) && !empty($product_data['pream_points']) ? $product_data['pream_points'] : "" )?>" required>
                            </div>
			            </div>
						<div class="col-md-12">
			            ...
			            <div class="col-md-12">
							...
							<div class="dropzone">
								...
								<input type="file" name="product_galary[]" class="fileupload" id="product_galary" multiple>
							</div>
				       ...
				       <!-- <div class="col-md-12">
	                        <div class="form-group">
				                <label>Meta Title <small> (or re-enter product name)</small></label>
				        ...
				        <!--<div class="col-md-12">
	                        <div class="form-group">
				                <label>Meta Description <small> (or re-enter product description)</small></label>
...
// 		$("#unit_price, #product_gst").keyup(function() {
...
// 	});
	...
	$('#product_gst_button').on('click', function(){
	    var p = $("#unitprice_plus_gst").val();
		var g = $("#product_gst").val();
        var k = (parseFloat ((p * g) / 100).toFixed(2));
		$("#gst_amount").val(k); 
		
	});
	
	 $('#product_gst_button').on('click', function(){
	    var a = $("#unitprice_plus_gst").val();
		var b = $("#gst_amount").val();
		// alert(g);
	...
	$("#front_price_button").on('click', function(){
		var up = $("#unitprice_plus_gst").val();
		var dic = $("#discount").val();
		var gstamt = $("#gst_amount").val();
		
        var selValue = $("#discount_type").val();
        // alert(up_gstamt);
		// alert(a);
		// alert(b);
		if(selValue == 1){
			// document.write("wow");
			var up_gstamt = Number(up) + Number(gstamt);
			var dic_t = (100 - Number(dic));
			var cal_fp = (parseFloat ((up_gstamt / dic_t) * 100).toFixed(0));
			// var per = (parseFloat ((up * dic) / 100).toFixed(2));
			// var fp = Number(up) + Number(per) + Number(gstamt);
			$("#unit_price").val(cal_fp);
			// alert(cal_fp);
		}if(selValue == 2){
			var fp = Number(up) + Number(dic) + Number(gstamt);

			$("#unit_price").val(fp);
		}else{
			var x = Number(up) + Number(gstamt);
			// alert(c);
			// alert("you are good");
		}
    });
...
// 	    var p = $("#unitprice_plus_gst").val();
// 		var g = $("#gst_amount").val();
// 		alert(g);
//         var k = (parseFloat ((p) +g)).toFixed(2));
//         alert(k);
// 		$("#amount_and_gst").val(k); 
...
// 	});
  ...
  function categorychange(){
    var cat_id = $("#category").val();
		    $.ajax({
			   type:"POST",
			   url:base_url+ 'shop/categoryhtml',
			   data:{categories_id:cat_id},
			   dataType:'json',
			   success: function(data){
			       $('#categoryimage').html("<label>Category Image</label><br><img src='<?php echo base_url();?>uploads/category/"+data.categorylist.category_image+"' style='width:491px;height:169px'><input type='hidden' name='categoryimg' value='"+data.categorylist.category_image+"'>");
				  //$('.subcategorylist').html(data);
			   }
		   });
  }
